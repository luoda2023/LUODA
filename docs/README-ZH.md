...
> [!警告]
> **免责声明:** <br>
> 白狐 的开发人员不纵容或支持任何不道德或非法的软件使用行为。滥用行为，例如未经授权的访问、控制或侵犯隐私，严格违反我们的准则。作者对应用程序的任何滥用行为概不负责。
...
白狐 期待各位的贡献. 如何参与开发? 详情请看 [CONTRIBUTING-ZH.md](CONTRIBUTING-ZH.md).
...
生成的可执行程序在 target 目录下，可直接通过指令运行调试 (Debug) 版本的 白狐:
```sh
...
请注意：
* 请保证您运行的目录是在 白狐 库的根目录内，否则软件会读不到文件。
